...
LLColor4U lggBeamMaps::beamColorFromData(lggBeamsColors data)
{
	...
	S32 diference = llmath::llround(data.endHue  - data.startHue);
...
void lggBeamMaps::fireCurrentBeams(LLPointer<LLHUDEffectSpiral> mBeam, LLColor4U rgb)
{
	...
	for(int i = 0; i < (int)dots.size(); i++)
	{
		...
		//LL_INFOS() << "dist is " << distanceAdjust << "scale is " << scale << LL_ENDL;
...
F32 lggBeamMaps::setUpAndGetDuration()
{
	...
	if(settingName != lastFileName)
	{
		...
		if(settingName != "===OFF===" && settingName != "")
		{
			...
			LL_INFOS() << "reading it all now size is " << myPicture.size() << " and duration is " << duration << LL_ENDL;
	...
	//LL_INFOS() << "sent final dur of " << duration << LL_ENDL;
