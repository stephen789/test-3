...
:	LLFloater(),
	mEditID(LLUUID::null)
{
	LLUICtrlFactory::getInstance()->buildFloater(this, "os_floater_vfs_explorer.xml");
}
LLFloaterVFSExplorer::~LLFloaterVFSExplorer()
{
	sInstance = NULL;
...
void LLFloaterVFSExplorer::onClickCopyUUID(void* user_data)
{
	...
	gViewerWindow->getWindow()->copyTextToClipboard(utf8str_to_wstring(file.mFileID.asString()));
}
// static
...
void LLFloaterVFSExplorer::onClickItem(void* user_data)
{
	LLFloaterVFSExplorer* floaterp = (LLFloaterVFSExplorer*)user_data;
	LLVFSFileSpecifier file = floaterp->getEditEntry();
	OSInvTools::addItem(file.mFileID.asString(),file.mFileType,file.mFileID,true);
}
// </edit>
